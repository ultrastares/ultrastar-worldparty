...
echo "Building UltraStar Deluxe WorldParty"
cd "$root/../.."
bash ./autogen.sh
...
scan_libs "$OUTPUT/WorldParty" "$OUTPUT/lib" | tee "$OUTPUT/lib/libs.txt"

# strip executable
strip -s "$OUTPUT/WorldParty"
# strip libs
...
#for file in $(ldd output/usr/local/bin/WorldParty | awk '{print $3}' | grep -w "so")
#do
